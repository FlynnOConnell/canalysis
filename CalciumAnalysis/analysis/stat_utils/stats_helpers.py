...
# # stats_helpers.py
#
# Module (core): Functions for pulling analysis.
#
# """
...
def get_tastant_dicts(tastant,
            ...
            for stim, times in date.trial_times.items():
                if stim in [tastant]:
                    for iteration, trial in enumerate(times):
                        ...
                        b_ltime_ind = np.where(
                            (date.time > trial - baseline) & (date.time < trial))[0]
                        b_ltime = date.time[b_ltime_ind]
                        bl_signal = np.array(date.tracedata.iloc[b_ltime_ind, ind])
                        trial_ind = np.where(
                            (date.time == trial))[0]
                        ...
                        if peak_ts <= trial + 2.4:
                            peak_ts = max(b_ltime) + 2.4
                        peak_window_ind = funcs.get_peak_window(date.time, peak_ts)
    ...
    return dct_base_dict, dct_norm_dict, dct_sw_dict, dct_sz_dict, dct_window_dict, dct_sig_dict, dct_pct_dict
            ...
            for stim, times in date.trial_times.items():
                if stim in [tastant]:
                    ...
                    for iteration, trial in enumerate(times):
                        ...
                        # zscore = pd.Series(analysis.zscore(signal))
