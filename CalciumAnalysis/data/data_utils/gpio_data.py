...
class GpioData:
    ...
    def get_rec_end(self):
        rec_end = max(self.gpiodata.loc[np.where(
            self.gpiodata[' Channel Name'] == ' BNC Sync Output')[0], 'Time (s)'])
        return rec_end
    
    def trim(self):
        self.gpiodata = self.gpiodata.iloc[np.where(
            self.gpiodata[' Channel Name'].str.contains('GPIO'))[0], :]
    
    def get_timestamps(self):
        for chan in pd.unique(self.gpiodata[' Channel Name']):
            print('Processing events for:', chan)
            event = [] 
            gp_chan = self.gpiodata.iloc[np.where(
                self.gpiodata[' Channel Name'] == chan)[0], :]
            ...
            for index, row in gp_check.iterrows():
                ...
                tcomp = gp_chan[(gp_chan['Time (s)'] >= t-.011) &
                                (gp_chan['Time (s)'] < t)][' Value'] > self.threshold
