...
def interval(lst: list) -> collections.Iterable:
...
def dup_check(signal, peak_signal) -> None:
    checker = []
    for value in signal:
...
def get_peak_window(time, peak_time) -> list:
...
def get_matched_time(time: pd.Series,
                     *argv: list,
...
# def match_samples(data, target_data):
