...
class Graph:
    ...
    def Scatter(x, y, z, label_x, label_y, label_z, session, pca_df):
        ...
        ax.scatter(x, y, z, s=40, c=pca_df.colors, marker='o', alpha=1)
        
        handles = [matplotlib.lines.Line2D([],[], marker="o", color=c, linestyle="none") for c in pca_df.colors.values]
        ax.legend(handles=handles, labels=list(pca_df.index.values), 
                   loc='upper right', prop={'size':6}, bbox_to_anchor=(1,1),ncol=2, numpoints=1)
        return fig

    
    def tsScatter(x, y, z, label_x, label_y, label_z, session, test_dir):
        """  Make scatter plot, given data and labels.  
        """
        fig = plt.figure()
        ax = fig.gca(projection='3d')
    ...
    def aniScatter(x, y, z, label_x, label_y, label_z, session, pca_df):
    ...
    def lineSession(nplot, tracedata, time, timestamps, lickshade, session, numlicks):
    ...
    def lineZoom(nplot, tracedata, time, timestamps, session, zoomshade, colors):
        # create a series of plots with a shared x axis
    ...
    def lineStim(nplot, tracedata, time, timestamps, trial_times, session, colors):
    ...
    def lineSingle(my_cell, cell_index, tracedata, time, timestamps, trial_times, session, colors):
        ...
        for stim, times in trial_times.items():
            ...
            for iteration, trial in enumerate(times):
                ...
                for stimmy in ['Lick', 'Rinse', stim]:
                    ...
                    timey = timestamps[stimmy]
                    for ts in timey:
                        if (ts > trial-1.5 and ts < trial+5):
                            if done == 0:
                                label = stimmy
                                done = 1
                            else:
